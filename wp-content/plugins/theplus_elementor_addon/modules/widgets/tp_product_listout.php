    ...
    protected function register_controls() {
	 ...
	 protected function render() {
		...
		if ( !class_exists('woocommerce') ) {
			...
			$output .= '<div id="pt-plus-product-list" class="product-list '.esc_attr($uid).' tp-pro-l-type-'.esc_attr($product_post_listing).' '.esc_attr($data_class).' '.$animated_class.'" '.$layout_attr.' '.$data_attr.' '.$animation_attr.' data-enable-isotope="1">';
		...
		if(!empty($settings["product_post_listing"]) && $settings["product_post_listing"]=='archive_listing'){
			...
			if(!empty($query_var['taxonomy']) && ($query_var['taxonomy']=='product_cat' || $query_var['taxonomy']=='product_tag')){
				...
				if(isset($query_var['taxonomy'])){					
					$query_args[$query_var['taxonomy']] = $query_var['taxonomy'];
				}
			}
