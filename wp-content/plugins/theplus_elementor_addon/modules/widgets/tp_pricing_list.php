		...
		if(!empty($menu_style) && $menu_style == "style_3"){
			...
			if(!empty($icon_type) && $icon_type == 'lottie'){
				...
				if($ext!='json'){
					...
					$food_img .='<lottie-player src="'.esc_url($settings['lottieUrl']['url']).'" style="display: '.esc_attr($lottiedisplay).'; width: '.esc_attr($lottieWidth).'px; height: '.esc_attr($lottieHeight).'px; margin-right: '.esc_attr($lottieMright).'px;" '.esc_attr($lottieLoopValue).'  speed="'.esc_attr($lottieSpeed).'" '.esc_attr($lottieAnim).'></lottie-player>';
				}
			}
		}	
