...
class ThePlus_Search_Filter extends Widget_Base {
    ...
    protected function register_controls() {
        ...
        $repeater->add_control('wlrb_tabbing',
			[
				'label'=>__('Work like Radio Button','theplus'),
				'type'=>Controls_Manager::SWITCHER,
	...
	protected function render() {
                    ...
                    $FilterField .= '<div class="field-col '.esc_attr($Titlelayout).' '.esc_attr($RepeaterClass).' '.esc_attr($desktop_class).' '.esc_attr($tablet_class).' '.esc_attr($mobile_class).' ">';
                        ...
                        if($FilterOption == 'wpfilter'){
                            if($Wp_Type == 'alphabet'){
                                ...
                                $MaxPrice = !empty($Filter['maxPrice']) ? $Filter['maxPrice'] : 10000;
                                $MinPrice = !empty($Filter['minPrice']) ? $Filter['minPrice'] : 0;
        ...
        $JSData = json_encode( array(
            ...
            'widgetid' => $uniqWidgetId,
            'enable_archive' => !empty($settings['enable_archive']) ? true : false,
        ), true);
            ...
            if(is_archive() && !\Elementor\Plugin::$instance->editor->is_edit_mode()){
                foreach($data as $value => $label){
                    ...
                    if( !empty($CatName) && strtolower($CatName) == strtolower($DataName) ){
                        ...
                        $ArchiveName = $value;
                        $output .= $this->tp_checkbox_html($Repeater, $label, $value, $TPPrefix, $name, $EnableChecked, $classArchive);
                    }
                }
                        ...
                        if( !empty($DataChild) ){
                            foreach($DataChild as $value => $label){
                                ...
                                if( !empty($CatName) && strtolower($CatName) == strtolower($DataName) ){
                                    $EnableChecked = "checked";
                                    $ArchiveName = $value;
                                    $SetID = !empty($label->term_id) ? $label->term_id : 1;
                                    $label = json_decode(json_encode($label), true);
                                    $output .= $this->tp_checkbox_html($Repeater, $label, $SetID, $TPPrefix, $name, $EnableChecked, $classArchive);
                                }
                            }
                        }
                    }
    ...
    protected function tp_checkbox_html($Repeater, $label, $value, $TPPrefix, $name, $EnableChecked, $classArchive){
        ...
        $output = '<div class="tp-checkBox '.esc_attr($layout_style).' '.esc_attr($classArchive).'">';
            $output .= '<div class="tp-group">';
                $output .= '<div class="tp-group-one">';
                    $output .= '<input type="checkbox" name="'.esc_attr($name).'" value="'.esc_attr($value).'" id="'.esc_attr($TPPrefix).esc_attr($value).'" data-title="'.esc_attr($DataName).'" '.$EnableChecked.'/>';
                        ...
                        if( !empty($DataChild) ){
                            foreach($DataChild as $value => $label){
                                ...
                                if( !empty($CatName) && strtolower($CatName) == strtolower($DataName) ){
                                    ...
                                    $SetID = !empty($label->term_id) ? $label->term_id : 1;
                                    $label = json_decode(json_encode($label), true);
                                    $output .= $this->tp_radio_html($Repeater, $label, $SetID, $TPPrefix, $name, $EnableChecked, $classArchive);
                                }
                            }
                        }
                    }
    ...
    protected function tp_tabbing_html($Repeater, $value, $key, $name, $ArchiveActive, $classArchive){
        ...
        $output = '<div class="tp-tabbing-wrapper '.$ArchiveActive.' '.$classArchive.'">';
            $output .= '<label class="tp-tabbing-item">';
            if(isset($Repeater['wlrb_tabbing']) && $Repeater['wlrb_tabbing']=='yes'){
                $tabtypeselection = 'radio';
            }else{
                $tabtypeselection = 'checkbox';
            }
            
                $output .= '<input type="'.$tabtypeselection.'" class="tp-tabbing-input" id="tp-'.esc_attr($key).'" name="'.esc_attr($name).'" value="'.esc_attr($key).'" data-title="'.esc_attr($DataName).'" '.$EnableChecked.'>';
                if($TabbingMedia == 'icon'){
                    if( !empty($Repeater['TabbingIconlib']) ){
                        ob_start();
                            \Elementor\Icons_Manager::render_icon( $Repeater['TabbingIconlib'], [ 'aria-hidden' => 'true' ]);
