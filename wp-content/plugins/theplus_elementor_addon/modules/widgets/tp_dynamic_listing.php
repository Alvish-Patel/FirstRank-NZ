...
class ThePlus_Dynamic_Listing extends Widget_Base {
	...
	protected function render() {
		...
		if ( ! $query->have_posts() && $settings["blogs_post_listing"] != 'acf_repeater' ) {			
			...
			$output .= '<div id="pt-plus-dynamic-listing" class="dynamic-listing '.esc_attr($uid).' '.esc_attr($data_class).' '.esc_attr($style_layout).' '.$animated_class.' '.$child_filter_class.' tp-dy-l-type-'.esc_attr($settings["blogs_post_listing"]).'" '.$layout_attr.' '.$data_attr.' '.$animation_attr.' data-enable-isotope="1">';
